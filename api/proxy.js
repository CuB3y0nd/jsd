...
export default async function handler(request) {
  try {
    ...
    if (githubRepo && !isGitHubRepoAllowed(githubRepo)) {
      return createErrorResponse(`仓库 ${githubRepo} 不被允许访问`, 403)
    }

    // 检查npm包
    const npmPackage = extractNpmPackage(targetUrl)
    if (npmPackage && !isNpmPackageAllowed(npmPackage)) {
      return createErrorResponse(`npm 包 ${npmPackage} 不被允许访问`, 403)
    }

    // 检查referer
    const referer = request.headers.get('referer')
    if (!isRefererAllowed(referer)) {
      return createErrorResponse('来源站点不被允许访问', 403)
    }
    ...
    if (CONFIG.MAX_FILE_SIZE > 0) {
      ...
      if (contentLength && parseInt(contentLength) > maxSize) {
        return createErrorResponse(`文件过大，超过 ${CONFIG.MAX_FILE_SIZE}MB 限制`, 413)
      }
